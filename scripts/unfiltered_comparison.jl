...
function experiment_error()::Matrix{Float64}
    ...
    for i = 1:size(Ns, 1)
        Yn = Y[:, 1 : Ns[i]]
        Un = U[:, 1 : Ns[i]]
        err[i, size(μs, 1) + 1] = opnorm(ordinary_least_squares(Yn, Un, T=T) - G)
    end
    err
end
...
p = plot(
         ...
         label=[L"\mu = 10^0" L"\mu = 10^1" L"\mu = 10^2" L"\mu = 10^3" L"\mu = 10^4" "OLS"],
         title=L"error with $\rho(A) = %$ρ$ and $\sigma_w, \sigma_z = %$σwz$",
        )
#savefig(p, "plots/unfiltered_rho_small.pdf")
savefig(p, "plots/unfiltered_rho_1me.pdf")
#savefig(p, "plots/unfiltered_rho_1pe.pdf")
#savefig(p, "plots/unfiltered_rho_big.pdf")
